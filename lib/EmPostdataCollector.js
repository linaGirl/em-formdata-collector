!function(){
	...
	module.exports = new Class({
		...
		request: function(request, response, next) {
			var   contentType = (request.hasHeader('content-type') ? request.getHeader('content-type') : '').toLowerCase().trim()
				, parts;
			...
			if (/multipart\/form-data/i.test(contentType)) {
				...
				request._reader.on('part', function(part) {
					...
					collector.on('end', function(fileData) {
						...
						else data = Buffer.isBuffer(fileData) ? fileData.toString() : '';
				...
				collector.on('end', function(buf) {
					...
					request._formData = data;
					request._formDataLoaded = true;
					request.emit('formDataLoaded');
				}.bind(this));

				// pipe the stuff
				request.getRequest().pipe(collector);
